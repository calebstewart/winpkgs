...
`winpkgs/scheduledTask`, `winpkgs/optionalFeature`, `winpkgs/groupMember`, and one each for the
...
name and the hash together, so it must match the file's own basename. It also
marks the file unfree, which Windows is, so `pkgs` here needs
`config.allowUnfree = true`; the one-command front ends allow it themselves.

Three more things go on the media, pinned and fetched the ordinary way since
their URLs do not expire: the NixOS-WSL image a system with `wsl.enable`
imports (it only bootstraps; the configuration's own system is substituted from
a cache on the media), the WSL MSI, and the `Microsoft.WinGet.Client` module
...
the first time it is run -- so the installer brings its own. Pass `wslRootfs`,
`wslMsi` or `wingetClient` to change a version, or `null` to leave one out.
Like the ISO, none is fetched by anything that merely evaluates the flake.
Packages installed through winget still need a network.
